...
$sql = "SELECT type, amount, category, expenseType, note, createdAt FROM TRANSACTION WHERE TID = ? AND author_UID = ? AND isDeleted = FALSE";
...
// Ensure the dropdown matches the transaction type
$transType = $transaction['type'];
$catStmt = $conn->prepare("SELECT name FROM user_category WHERE UID = ? AND type = ? ORDER BY name");
$catStmt->bind_param("is", $uid, $transType);
$catStmt->execute();
$categories = $catStmt->get_result();

// --- 2. Check 24-hour window (86400 seconds) ---
$secondsPassed = time() - strtotime($transaction['createdAt']);
$isLocked = ($secondsPassed > 86400) ? true : false;

// --- 3. Handle Update ---
if ($_SERVER["REQUEST_METHOD"] == "POST" && !$isLocked) {
    $newAmount = $_POST['amount'];
    $newCategory = $_POST['category'];
    // Only update expenseType if it was originally an expense
    $newType = isset($_POST['expenseType']) ? $_POST['expenseType'] : NULL;
...
<head>
    ...
    <title>Edit Transaction | Remainder</title>
    <style>
        /* Exact same CSS as your original edit_transaction.php */
        :root { --primary: #1e293b; --accent: #3b82f6; --success: #22c55e; --warning: #f59e0b; --danger: #ef4444; --bg: #f8fafc; --card: #ffffff; --text: #334155; --border: #e2e8f0; }
        ...
        .edit-card { background: var(--card); padding: 35px; border-radius: 20px; box-shadow: 0 10px 25px -5px rgba(0, 0, 0, 0.1); width: 100%; max-width: 450px; }
        h2 { margin: 0 0 10px 0; color: var(--primary); font-size: 1.5rem; text-align: center; }
        .time-banner { background-color: #eff6ff; border: 1px solid #dbeafe; color: #1e40af; padding: 12px; border-radius: 10px; font-size: 0.85rem; margin-bottom: 25px; text-align: center; }
        .locked-error { background-color: #fee2e2; color: var(--danger); border: 1px solid #fecaca; padding: 20px; border-radius: 12px; text-align: center; }
        ...
        .btn-update { width: 100%; background-color: var(--primary); color: white; padding: 14px; border: none; border-radius: 10px; font-size: 1rem; font-weight: 600; cursor: pointer; transition: 0.2s; margin-top: 10px; }
                ...
                <select name="category" required>
                    <?php if($categories->num_rows > 0): ?>
                        <?php while($cat = $categories->fetch_assoc()): ?>
                            <option value="<?= htmlspecialchars($cat['name']) ?>" <?= $transaction['category'] == $cat['name'] ? 'selected' : '' ?>>
                                <?= htmlspecialchars($cat['name']) ?>
                            </option>
                        <?php endwhile; ?>
                    <?php else: ?>
                        <option value="<?= htmlspecialchars($transaction['category']) ?>" selected><?= htmlspecialchars($transaction['category']) ?> (Legacy)</option>
                    <?php endif; ?>
                </select>
                <a href="manage_categories.php" class="manage-cat-link">+ Manage Categories</a>
            </div>

            <?php if($transType == 'Expense'): ?>
            <div class="form-group">
